#!/usr/bin/env python3
"""Builds the README's visuals: hero-{light,dark}.svg and structuring-{light,dark}.svg.
...
The picture is the protocol. Two lights, cyan for the effector's receipt (witness 1) and
amber for the Flare Data Connector (witness 2), drift together, and where they agree the
glass turns white: that overlap is the evidence.
"""
import os
...
def hero(theme, H=440, R=28):
    c = THEMES[theme]
    W = 1280
    dy = (H - 440) / 2
    sx, sy, sw, sh = 330, 118, 620, 204           # the glass slab
    cyL, cyR, cyy = 500, 780, 220                 # the two lights, at rest
    out = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" role="img" '
           f'aria-label="DELICTI — accountability for autonomous AI agents. Two witnesses, one deed.">',
           "<style>",
           ".cy{animation:cy 14s cubic-bezier(.45,0,.55,1) infinite}"
           ".am{animation:am 14s cubic-bezier(.45,0,.55,1) infinite}"
           ".co{animation:co 14s cubic-bezier(.45,0,.55,1) infinite}"
           ".pulse{animation:pu 14s cubic-bezier(.45,0,.55,1) infinite}"
           "@keyframes cy{0%,100%{transform:translateX(-70px)}50%{transform:translateX(70px)}}"
           "@keyframes am{0%,100%{transform:translateX(70px)}50%{transform:translateX(-70px)}}"
           "@keyframes co{0%,18%,82%,100%{opacity:0}50%{opacity:1}}"
           "@keyframes pu{0%,30%,70%,100%{opacity:.35}50%{opacity:1}}",
           REDUCED, "</style>",
           "<defs>",
           '<filter id="soft" x="-80%" y="-80%" width="260%" height="260%"><feGaussianBlur stdDeviation="46"/></filter>',
           '<filter id="softer" x="-80%" y="-80%" width="260%" height="260%"><feGaussianBlur stdDeviation="30"/></filter>',
           f'<filter id="lift" x="-20%" y="-30%" width="140%" height="180%"><feDropShadow dx="0" dy="22" stdDeviation="26" '
           f'flood-color="#000" flood-opacity="{c["shadow_op"]}"/></filter>',
           f'<linearGradient id="rim" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="{c["rim"]}" stop-opacity="{c["rim_op"]}"/>'
           f'<stop offset=".5" stop-color="{c["rim"]}" stop-opacity="{c["rim_op"] * 0.25:.3f}"/>'
           f'<stop offset="1" stop-color="{c["rim"]}" stop-opacity="{c["rim_op"] * 0.6:.3f}"/></linearGradient>',
           f'<linearGradient id="sheen" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#FFF" stop-opacity="{0.35 if theme == "light" else 0.07}"/>'
           f'<stop offset=".45" stop-color="#FFF" stop-opacity="0"/></linearGradient>',
           f'<radialGradient id="vig" cx=".5" cy=".5" r=".75"><stop offset="0" stop-color="{c["bg"]}" stop-opacity="0"/>'
           f'<stop offset="1" stop-color="{c["bg"]}" stop-opacity="1"/></radialGradient>',
           f'<clipPath id="card"><rect width="{W}" height="{H}" rx="{R}"/></clipPath>',
           "</defs>",
           '<g clip-path="url(#card)">',
           f'<rect width="{W}" height="{H}" rx="{R}" fill="{c["bg"]}"/>',
           dots(c, W, H, R), f'<g transform="translate(0 {dy})">']
    # a field of tiles — the slab is one of many; the lights bleed into its neighbours
    tile, gap = 148, 12
    for col in range(-1, 9):
        for row in range(-2, 4):
            x, y = 30 + col * (tile + gap), -34 + row * (tile + gap) + 44
            out.append(f'<rect x="{x}" y="{y}" width="{tile}" height="{tile}" rx="22" fill="{c["tile_fill"]}" '
                       f'fill-opacity="{c["tile_fill_op"]}" stroke="{c["tile_stroke"]}" stroke-opacity="{c["tile_op"]}"/>')
    # the two witnesses, and the white where they agree
    out += [f'<g class="cy"><circle cx="{cyL}" cy="{cyy}" r="150" fill="{c["cyan"]}" fill-opacity="{c["light_op"]}" filter="url(#soft)"/></g>',
            f'<g class="am"><circle cx="{cyR}" cy="{cyy}" r="150" fill="{c["amber"]}" fill-opacity="{c["light_op"]}" filter="url(#soft)"/></g>',
            f'<g class="co"><ellipse cx="{W / 2}" cy="{cyy}" rx="120" ry="92" fill="{c["core"]}" fill-opacity="{c["core_op"]}" filter="url(#softer)"/></g>',
            f'<rect y="{-dy}" width="{W}" height="{H}" rx="{R}" fill="url(#vig)" fill-opacity=".55"/>']
    # the slab
    out += [f'<g filter="url(#lift)"><rect x="{sx}" y="{sy}" width="{sw}" height="{sh}" rx="30" fill="{c["glass"]}" fill-opacity="{c["glass_op"]}"/></g>',
            f'<rect x="{sx}" y="{sy}" width="{sw}" height="{sh}" rx="30" fill="url(#sheen)"/>',
            f'<rect x="{sx + .75}" y="{sy + .75}" width="{sw - 1.5}" height="{sh - 1.5}" rx="29.25" fill="none" stroke="url(#rim)" stroke-width="1.5"/>',
            t("DELICTI", "Geist-Medium", 92, W / 2, 238, c["ink"], anchor="middle", track=0.16),
            t("Accountability for autonomous AI agents", "Geist-Regular", 18, W / 2, 282, c["ink2"], anchor="middle", track=0.01)]
    # schematic annotations, set small, as on an instrument
    mono = "GeistMono-Medium"
    ln = f'stroke="{c["ink3"]}" stroke-width="1" fill="none"'
    out += [t("WITNESS 01", mono, 11, 64, 58, c["ink2"], track=0.14),
            t("EFFECTOR RECEIPT", mono, 11, 64, 76, c["ink3"], track=0.14),
            f'<path d="M 204 71 H {cyL - 70} V {sy - 10}" {ln}/>',
            f'<circle class="pulse" cx="{cyL - 70}" cy="{sy - 10}" r="3" fill="{c["cyan"]}"/>',
            t("WITNESS 02", mono, 11, W - 64, 58, c["ink2"], anchor="end", track=0.14),
            t("FLARE DATA CONNECTOR", mono, 11, W - 64, 76, c["ink3"], anchor="end", track=0.14),
            f'<path d="M {W - 240} 71 H {cyR + 70} V {sy - 10}" {ln}/>',
            f'<circle class="pulse" cx="{cyR + 70}" cy="{sy - 10}" r="3" fill="{c["amber"]}"/>',
            f'<path d="M {W / 2} {sy + sh + 10} V {sy + sh + 40}" {ln}/>',
            t("AGREEMENT IS EVIDENCE", mono, 11, W / 2, sy + sh + 60, c["ink3"], anchor="middle", track=0.14),
            t("MANDATE · DEED · BOND", mono, 11, 64, 400, c["ink3"], track=0.14),
            t("FLARE · XRPL · SPEC 1.0", mono, 11, W - 64, 400, c["ink3"], anchor="end", track=0.14),
            "</g></g></svg>"]
    return "\n".join(out)
...
if __name__ == "__main__":
    for th in THEMES:
        for name, fn in (("hero", hero), ("structuring", structuring)):
            p = os.path.join(OUT, f"{name}-{th}.svg")
            open(p, "w").write(fn(th))
            print(p, os.path.getsize(p))
    # 1280×640 for GitHub's social preview (Settings → Social preview); rasterise it with any browser
    open(os.path.join(OUT, "social-preview.svg"), "w").write(hero("dark", H=640, R=0))
