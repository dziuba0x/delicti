...
**On a docket.** `fileBudgetPayments` is the same case filed the way §6.10 is. Each receipted payment goes on the mandate's `paymentDocket` once (`paymentFiled` by transaction id, `receiptFiled` by leaf, since one receipt accounts for one payment across filings too). Below the budget a filing only records, with no commitment. The filing that crosses is the committed conviction (kind 6). Already-filed payments are skipped, not refused. A deed on XRPL can then be proven while the verifier still remembers it and counted after it no longer does. The one-shot `challengeBudgetOverrunPayment` stays, and both are nested in the budget bucket (§8.1), so they cannot double-count.
...
**Batch (XLS-56), measured on devnet 2026-09-23.** `BatchV1_1` is enabled on devnet, not yet on the testnet the FDC indexes, and is expected on mainnet around 29.09.2026. In a Batch the XRP does not leave in the outer transaction, which pays only the fee (−4 drops in the run). It leaves in the **inner** transactions. Each inner transaction is a ledger transaction of its own, with its own hash, `Fee` 0, no signature, flag `tfInnerBatchTxn`, and `meta.ParentBatchID` pointing at the outer one (outer `42C6E2E9…`, inner `3793EAA5…` −3 XRP and `E8801688…` −2 XRP). `account_tx` lists them, so a watcher sees them. **Unverified:** whether the FDC verifier attests an inner transaction at all. It has no signature and no fee, and a verifier that expects either may refuse it. If it refuses, Batch is a hole in §6.10: outflow the FDC cannot see. `tools/xrpl_testnet.py batch` reproduces the run and is the test to rerun the day the testnet enables the amendment. Until then §10 lists it.
...
### 6.11 Gross ERC-20 outflow (`JudgeEvm.fileErc20Outflow`) (v0.13)

The EVM twin of §6.10, written for the agent the stablecoin market will actually run. x402 on EVM settles by EIP-3009 `transferWithAuthorization`. The agent **signs** and a facilitator **sends**, so the agent is never a transaction's sender. An agent that also writes no receipts was reachable only by §6.4, one staked accusation per deed. Under §6.11 its budget is enforced from the token's own event log.

**Who it applies to.** Mandates whose `assetKey` is an ERC-20 (a left-padded address, `Deeds.erc20Of`) on an EVM source, and whose agent declared the mandate **exclusive** (`MandateRegistry.declareExclusive`). With no receipts, only that promise makes everything leaving the address this mandate's business. The EVM key is the right key here: on an EVM source the agent *is* that address.
...
**The verifier's memory is not the reason here.** The FDC's EVM verifier has no lower timestamp limit on Flare and Songbird, and about 300 days on Ethereum. On EVM the docket exists for the receipt-less conviction and for the per-log anti-burial rule. §6.10 needs it for a different reason: its verifier forgets.

**What it covers:** USDT0, USDC.e, FXRP and any standard ERC-20 on Flare, Songbird and Ethereum; x402 settlements (EIP-3009), `transfer`, `transferFrom` on an allowance, `permit` flows, burns. **What it does not:** chains the FDC does not index (Base, where most x402 volume settles today; other L2s); stablecoins on XRPL (RLUSD); native FLR (that is §6.2 with receipts, §6.5 with a meter).
...
- **XRPL deeds must be proven within ~14 days of happening.** The FDC's XRP verifier indexes about 15 days. Under an outflow budget the docket (§6.10, v0.12) carries a deed once it is filed, so long windows are enforceable if somebody keeps filing. §6.8 has a docket as well (`fileBudgetPayments`). Either way a deed that nobody files within ~14 days of it is lost to the case, and `JudgeXrpl.fullyEnforceable` says whether a mandate's whole window fits inside that.
