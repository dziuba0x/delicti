...
Every challenge type DELICTI defined up to v0.8 has been executed on Coston2, in both directions where it has two: an accusation that stands and one that is answered, a challenge that lands and a copy of it that is refused. v0.9's XRPL challenge has not been — see below.
...
| **v0.9** — mandate pins `sourceId` / `assetKey` / `agentRef` / its own `bond`; `acknowledge`; XRPL overrun on FDC `Payment` proofs + `AgentRefs`; proportional, cumulative slashing with pro-rata return of deposits; `CorroborationLog`, `Deeds`, `BondLens`; invariant campaign | **deployed; native overrun executed on Coston2** — XRPL path, corroboration and second verdicts: tests only |
